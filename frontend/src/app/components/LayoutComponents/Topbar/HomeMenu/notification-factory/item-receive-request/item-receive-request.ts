...
@Component({
  selector: 'app-item-receive-request',
  templateUrl: './item-receive-request.html',
  styleUrls: ['../requests.scss']
})
export class ItemReceiveNotificationComponent implements OnInit {
  ...
  whenApproveClicked(item,e) {
    ...
    .pipe(
      switchMap(data=>{
        ...
        forkJoin([
          this.http.post(`/issueproc/issueProcessingForReqDocumentItemRequest`,{DOCUMENT_ID:this.notificationData.REQUEST_ITEM_RECEIVE_ID}).subscribe()
        ]),of(data)
        )
      })
    ).subscribe(
