...
export class StoresItemsGroupNoModelService extends TableBase{
  ...
  public searchItems(searchText: string): void {
    ...
    if (searchText) {
      ...
      let isTextInSTORES_ITEMS_GROUP_NO_ID = (item: any) =>
        item.STORES_ITEMS_GROUP_NO_ID.toString()
          .toLowerCase()
      ...
      this.displayData = this.savedData.filter(
        item => isTextInSTORES_ITEMS_GROUP_NO_ID(item) || isTextInSTORE_EN_NAME(item) || isTextInSTORE_AR_NAME(item) || isTextInITEM_GROUP_EN_NAME(item) || isTextInITEM_GROUP_AR_NAME(item),
