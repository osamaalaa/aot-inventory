...
@Injectable()
export class StoresItemsNoModelService extends TableBase{
    ...
    public searchItems(searchText: string): void {
        if (searchText) {
            let isTextInSTORES_ITEMS_NO_ID = (item: any) =>
                item.STORES_ITEMS_NO_ID.toString()
                    .toLowerCase()
            ...
            this.displayData = this.savedData.filter(
                item =>
                isTextInSTORES_ITEMS_NO_ID(item) ||
