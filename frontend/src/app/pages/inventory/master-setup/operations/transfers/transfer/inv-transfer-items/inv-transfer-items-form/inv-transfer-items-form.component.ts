...
import { INV_TRANSFER_ITEMS_VALIDATION_MESSAGES } from './inv-transfer-items.validations.messages';
...
@Component({
  selector: 'app-inv-transfer-items-form',
  templateUrl: './inv-transfer-items-form.component.html',
  styles: [
    `
...
export class InvTransferItemsFormComponent implements OnInit {
  ...
  createForm():void {
    this.form = this._fb.group({
      INV_TRANSFER_ID:[this.INV_TRANSFER_ID],
      INV_TRANSFER_ITEMS_ID:[null],
  ...
  resetUnitQtyControl(){
    this.form.controls.UNIT_QUANTITY.reset();
  }
  ...
  validateUnitQuantity(control: AbstractControl) {
    ...
    .getOneTransfer(this.INV_TRANSFER_ID).pipe(
      switchMap(res=>{
          let STORES_ID = res.rows[0].STORES_ID;
          return this.operationsService.getQuantityOfItem(STORES_ID,ITEMS_ID,UNITS_ID)
      })
    )
    .map(res => {
